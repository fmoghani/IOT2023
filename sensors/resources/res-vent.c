...
#define LOG_MODULE "Temperature Actuator"
...
static void
res_post_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size,
                     int32_t *offset);

static void res_event_handler(void);

/* An actuator, depending on the post variable mode, the ventilation is activated or deactivated */
EVENT_RESOURCE(res_vent,
"title=\"Ventilation controller: ?POST/PUT mode=HEAT|COOL|OFF automatic=ON|OFF\";rt=\"Temperature Control\";obs",
res_get_handler,
...
res_post_put_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size,
    ...
    if ((len = coap_get_post_variable(request, "mode", &mode))) {
        ...
        if (strncmp(mode, "HEAT", len) == 0) {
            switch_heater_on();
        } else if (strncmp(mode, "COOL", len) == 0) {
            switch_cooler_on();
        } else if (strncmp(mode, "OFF", len) == 0) {
            switch_ventilator_off();
        } else {
            success = 0;
        }
        ...
        if (strncmp(mode, "AUTO", len) == 0) {
            set_vent_automatic();
        } else if (strncmp(mode, "MANUAL", len) == 0) {
            set_vent_manual();
        } else {
            success = 0;
        }
...
static void res_get_handler(coap_message_t *request, coap_message_t *response,
    ...
    if (accept == TEXT_PLAIN) {
        ...
        snprintf((char *) buffer, COAP_MAX_CHUNK_SIZE, "mode=%d,timestamp=%lu,automatic=%d",
                 get_vent_mode(), timestamp, vent_is_automatic());
...
static void res_event_handler(void) {
    ...
    coap_notify_observers(&res_vent);
}
