...
/* A simple getter example. Returns the reading from temp sensor with a simple etag */
EVENT_RESOURCE(res_temp,
"title=\"Temperature Control\";rt=\"Temperature Sensor\";obs",
res_get_handler,
NULL,
NULL,
NULL,
...
static void res_get_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset)
{
    ...
    double temp_value = collect_temp_data();
    ...
    if(accept == TEXT_PLAIN) {
        ...
        snprintf((char *)buffer, COAP_MAX_CHUNK_SIZE, "temp=%f,timestamp=%lu", temp_value, timestamp);
